...
class game(models.Model):
    game_id = models.CharField(
        max_length=32, default="id", null=False, blank=False, primary_key=True
    )
    ...
    instructor = models.ForeignKey(
        User, limit_choices_to={"is_instructor": True}, on_delete=models.CASCADE
    )
...
class DemandPattern(models.Model):
    demand_id = models.CharField(
        max_length=32, null=False, default="id", blank=False, primary_key=True
    )
    weeks_num = models.IntegerField(blank=False, null=False)
    demands = models.CharField(
        max_length=128, null=False, blank=False, validators=[int_list_validator]
    )
    instructor = models.ForeignKey(
        User, limit_choices_to={"is_instructor": True}, on_delete=models.CASCADE
    )
...
class PlayerGame(models.Model):
    ...
    game_id = models.CharField(
        max_length=32, default="no-game", null=False, blank=False
    )
...
class Weeks(models.Model):
    ...
    demand = models.IntegerField(default=0, null=False, blank=False)
    incoming = models.IntegerField(default=0, null=False, blank=False)
    outgoing = models.IntegerField(default=0, null=False, blank=False)
