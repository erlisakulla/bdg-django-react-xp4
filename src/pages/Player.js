...
function Player() {
  ...
  let handlejoin = ()=>{
    axios.get('http://localhost:8000/api-game/'+joinGameID)
...
let handlecodechange =(e)=>{
  ...
  return (
    ...
    <div className="input-field col s6">
      <input name="gamecode"  id="gamecode" type="text" onChange={handlecodechange} className="validate"/>
      <label className="active" htmlFor="gamecode">Game Code</label>
    </div>
    <div className="input-field col s2">
      <button className="btn" onClick={handlejoin} >submit</button>
    </div>
  </div>
...
function Selectrole(props){
  ...
  <div className="input-field col s2">
      <button className="btn" onClick={handlerole} >submit</button>
    </div>
